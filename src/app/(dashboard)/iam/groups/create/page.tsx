...
type Step = "details" | "permissions" | "users" | "tags" | "review";

/**
 * Página para la creación de un nuevo grupo
...
export default function CreateGroupPage() {
  ...
  const availableUsers = [
    ...
    { id: "u3", username: "msmith", name: "María Smith" },
    { id: "u4", username: "rperez", name: "Roberto Pérez" },
    { id: "u5", username: "agarcia", name: "Ana García" },
  ];
  ...
  const handleUserChange = (userId: string) => {
    if (groupData.users.includes(userId)) {
      setGroupData({
        ...groupData,
        users: groupData.users.filter((id) => id !== userId),
      });
    } else {
      setGroupData({
        ...groupData,
  ...
  const handlePermissionChange = (permissionId: string) => {
    if (groupData.permissions.includes(permissionId)) {
      setGroupData({
        ...groupData,
        permissions: groupData.permissions.filter((id) => id !== permissionId),
      });
    } else {
      setGroupData({
        ...groupData,
        permissions: [...groupData.permissions, permissionId],
      });
    }
  };
  ...
  const addTag = () => {
    setGroupData({
      ...groupData,
      tags: [...groupData.tags, { key: "", value: "" }],
    });
  };

  const updateTag = (index: number, field: "key" | "value", value: string) => {
    const newTags = [...groupData.tags];
  ...
  const removeTag = (index: number) => {
    setGroupData({
      ...groupData,
      tags: groupData.tags.filter((_, i) => i !== index),
    });
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Redireccionar a la página de detalles del grupo creado
      router.push(`/iam/users/groups/${groupData.name}`);
    } catch (error) {
  ...
  // Validación básica
  const isDetailsValid = groupData.name.trim() !== "";

  const getUserByName = (userId: string) => {
    return availableUsers.find((user) => user.id === userId)?.username || "";
  };
        ...
        return (
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-300 mb-6">
              Ingrese la información básica del grupo.
            </p>
                ...
                <div className="flex">
                  ...
                  <div className="ml-3">
                    <p className="text-sm text-yellow-700 dark:text-yellow-400">
                      Debe proporcionar un nombre para el grupo.
                    </p>
                  </div>
                </div>
              </div>
      ...
      case "permissions":
        return (
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-300 mb-6">
              Seleccione las políticas que desea adjuntar a este grupo.
            </p>
              ...
              {availablePolicies.map((policy) => (
                <div key={policy.id} className="p-4 flex items-start">
                  <input
                    type="checkbox"
                    id={`policy-${policy.id}`}
                    checked={groupData.permissions.includes(policy.id)}
      ...
      case "users":
        return (
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-300 mb-6">
              Seleccione los usuarios que desea añadir a este grupo.
            </p>

            <div className="border border-gray-200 dark:border-gray-700 rounded-lg divide-y divide-gray-200 dark:divide-gray-700">
              {availableUsers.map((user) => (
                <div key={user.id} className="p-4 flex items-start">
                  <input
                    type="checkbox"
                    id={`user-${user.id}`}
                    checked={groupData.users.includes(user.id)}
      ...
      case "review":
        return (
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-300 mb-6">
              Revise la configuración del grupo antes de crearlo.
            </p>
            ...
            <div className="bg-gray-50 dark:bg-[#131e32] border border-gray-200 dark:border-gray-700 rounded-lg p-4 mb-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2 mb-4">
                    ...
                    {groupData.description && (
                      <p>
                        ...
                        <span className="text-sm text-gray-900 dark:text-white block mt-1">
                          {groupData.description}
                        </span>
                      </p>
                    )}
                ...
                <div>
                      ...
                      {groupData.users.length > 0 ? (
                        <ul className="text-sm text-gray-900 dark:text-white list-disc pl-5">
                          {groupData.users.map((userId) => (
                            <li key={userId}>{getUserByName(userId)}</li>
                          ))}
                        </ul>
                      ) : (
                        <p className="text-sm text-gray-500 dark:text-gray-400 italic">
                  ...
                  <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">
                    Etiquetas (
                    {groupData.tags.filter((tag) => tag.key.trim()).length})
                  </h3>
                  {groupData.tags.some((tag) => tag.key.trim()) ? (
              ...
              <div className="bg-red-50 dark:bg-red-900/20 text-red-800 dark:text-red-300 p-3 rounded-lg mb-6 flex items-start">
                ...
                <div>
                  ...
                  <ul className="list-disc list-inside mt-1 text-sm">
                    <li>El nombre del grupo es obligatorio</li>
                  </ul>
                </div>
              </div>
            )}
  ...
  const renderNavButtons = () => {
    const isNextDisabled =
      ...
      (currentStep === "review" && !groupData.name);
    ...
    return (
      <div className="flex justify-between mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
        ...
        >
          {currentStep === "review" ? "Crear grupo" : "Siguiente"}
  ...
  return (
    <div className="p-4 md:p-6 w-full">
      ...
      <div className="flex items-center text-sm mb-4">
        ...
        <ArrowRight className="h-4 w-4 mx-2 text-gray-400" />
        <span className="text-gray-600 dark:text-gray-400">Crear grupo</span>
      </div>
          ...
          <div className="flex flex-col items-center">
            <div
              className={`h-8 w-8 rounded-full flex items-center justify-center ${
                currentStep === "users"
                  ? "bg-blue-500 text-white"
                  : currentStep === "details" || currentStep === "permissions"
                  ? "bg-gray-300 dark:bg-gray-700 text-gray-600 dark:text-gray-400"
                  : "bg-green-500 text-white"
              }`}
            >
